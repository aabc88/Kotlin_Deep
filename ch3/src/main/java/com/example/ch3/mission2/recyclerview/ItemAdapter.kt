...
import com.example.ch3.mission2.fragments.WebFragment
import com.example.ch3.mission2.model.Item


class ItemAdapter(val context: Context, val datas: List<Item>, val callback: (String) -> Unit) :
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        holder.binding.run {
            ...
            visit.setOnClickListener {
                ...
                //생성자 매개변수로 전달된 callback 함수만 호출해서.. callback에서 화면 전환 처리 되게
                callback(item.url)
            }
            Glide.with(context)
